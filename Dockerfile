...
RUN mkdir diamond && cd diamond && wget https://github.com/bbuchfink/diamond/releases/download/v0.9.34/diamond-linux64.tar.gz && tar xvf diamond-linux64.tar.gz
RUN wget https://github.com/samtools/samtools/releases/download/1.10/samtools-1.10.tar.bz2
RUN tar -vxjf samtools-1.10.tar.bz2
RUN cd samtools-1.10 && ./configure --prefix=/samtools && make && make install
RUN git clone https://github.com/danielfrg/tsne.git && cd tsne && python setup.py install
RUN git clone https://github.com/KwanLab/Autometa && cd Autometa/pipeline && python setup_lca_functions.py build_ext --inplace
